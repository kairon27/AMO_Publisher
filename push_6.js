(function() {
  ...
  // Дефолтні налаштування (використовуються, якщо база не відповідає)
  let settings = {
    delay_seconds: 5,
    auto_show: true,
  ...
  // === 1. ЗАВАНТАЖЕННЯ НАЛАШТУВАНЬ (З ДІАГНОСТИКОЮ) ===
  async function loadSettings() {
    try {
      const hostname = window.location.hostname;
      console.log('🔍 [Push Debug] Поточний домен:', hostname);

      const url = `${CONFIG.SUPABASE_URL}/rest/v1/push_settings?site_domain=eq.${hostname}&select=*`;
      console.log('📡 [Push Debug] Роблю запит до:', url);
      ...
      if (!res.ok) {
        console.error('❌ [Push Debug] Помилка HTTP:', res.status, res.statusText);
        return;
      }

      const data = await res.json();
      console.log('📥 [Push Debug] Відповідь бази:', data);

      if (data && data.length > 0) {
        settings = { ...settings, ...data[0] };
        console.log('✅ [Push Debug] Налаштування застосовано!', settings);
      } else {
        console.warn('⚠️ [Push Debug] Налаштувань для цього домену не знайдено в базі. Використовую дефолтні.');
        console.warn('   Перевір: в адмінці домен має бути точно:', hostname);
      }
    } catch (e) {
      console.error('❌ [Push Debug] Критична помилка завантаження:', e);
    }
  }
  ...
  function showModal() {
    if (localStorage.getItem('push_denied_time')) {
        ...
        if (Date.now() - deniedTime < 24 * 60 * 60 * 1000) {
            console.log('⏸ [Push Debug] Користувач відмовився менше 24г тому.');
            return;
        }
    }
  ...
  async function requestSubscription() {
    ...
    try {
      ...
      console.log('✅ [Push Debug] Subscribed:', subscription);
      ...
      console.error('❌ [Push Debug] Subscription error:', err);
  ...
  async function init() {
    ...
    if ('serviceWorker' in navigator) {
        ...
        if (reg) {
            ...
            if (sub) {
                console.log('✅ [Push Debug] Користувач вже підписаний.');
                return; 
            }
        }
    }
    ...
    if (settings.auto_show && Notification.permission !== 'denied') {
        console.log(`⏱ [Push Debug] Вікно з'явиться через ${settings.delay_seconds} сек.`);
  ...
  (function trackClick() {
    ...
    if (cId && !sessionStorage.getItem('pc_' + cId)) {
       console.log('🚀 [Push Debug] Клік по пушу:', cId);
