(function() {
  ...
  // --- WAIT FOR SW ACTIVATION (CRITICAL FOR OPERA) ---
  async function waitForSWActivation(registration) {
      if (registration.active) return; // Вже активний

      const serviceWorker = registration.installing || registration.waiting;
      if (!serviceWorker) return;

      console.log('⏳ [Push] Waiting for SW activation state change...');
  ...
  async function requestSubscription() {
    ...
    try {
      console.log(`👷 [Push ${VERSION}] Starting registration...`);
      
      // 1. Register
      const registration = await navigator.serviceWorker.register('/sw_1.js');
      
      // 2. FORCE WAIT FOR ACTIVATION
      await waitForSWActivation(registration);
      
      // 3. WAIT FOR READY
      const readyReg = await navigator.serviceWorker.ready;
      console.log(`✅ [Push ${VERSION}] SW Ready.`);

      // 4. CHECK CONTROLLER (Opera Bug Fix)
      // Якщо SW ще не контролює сторінку, чекаємо трохи
      if (!navigator.serviceWorker.controller) {
          console.warn('⚠️ [Push] Page not controlled yet. Waiting...');
          await new Promise(r => setTimeout(r, 1000));
      }

      // 5. CHECK PUSH MANAGER
      if (!readyReg.pushManager) {
          console.warn('⚠️ [Push] pushManager missing. Updating registration...');
          await readyReg.update();
      }
      ...
      if (err.message && err.message.includes('push service error')) {
        alert('Помилка підписки в Opera. Спробуйте вимкнути "Block Trackers" для цього сайту.');
      }
    }
  }
  ...
  async function init() {
    ...
    if ('serviceWorker' in navigator) {
        try {
            // Перевіряємо наявність реєстрації БЕЗ виклику subscribe (щоб не лякати Opera)
            const reg = await navigator.serviceWorker.getRegistration();
            if (reg) {
                const sub = await reg.pushManager.getSubscription();
                if (sub) {
                    console.log(`✅ [Push ${VERSION}] Already subscribed (User: ${sub.endpoint.slice(-8)}).`);
                    return; // Вже підписані - виходимо, модалку не показуємо
                }
            }
        } catch (e) {
            console.warn('⚠️ [Push] Check error:', e);
        }
    }
    
    // Якщо дійшли сюди - підписки немає
